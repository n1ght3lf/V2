...
void openFiles(const vector<string>& filenames, bool Median) {
    for (const auto& inputFileName : filenames) {
        ...
        cout << "Laikas sugaištas duomenų nuskaitymui: " << std::fixed << std::setprecision(6) << readTime << " sekundės" << endl;
        cout << "Laikas sugaištas duomenų rušiavimui: " << std::fixed << std::setprecision(6) << sortTime << " sekundės" << endl;
        cout << "Laikas sugaištas duomenų išvedimui į du failus: " << std::fixed << std::setprecision(6) << outputTime << " sekundės" << endl;
    }
}
